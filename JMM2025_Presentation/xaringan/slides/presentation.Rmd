...
TBA: picture of all people and funding sources
...
---
# Example (To do: make better graph images)

![:img 40%, 10%, 20%](../figs/dowker1.png)

![:img 40%, 10%, 70%](../figs/dowker1_pers.png)

--

![:img 40%, 50%, 20%](../figs/dowker2.png)
...
- Chowdhury and Memoli prove:

Cycle graph with edge weights 1 $\xrightarrow{}$ induced cycle network from shortest path distance $\xrightarrow{}$ dim 1 barcode = $(1,$ $\text{ceil}(\frac{n}{2}))$

- We conjecture (with data):

Cycle graph with any edge weights $\xrightarrow{}$ induced cycle network from shortest path distance $\xrightarrow{}$ dim 1 barcode

$$\underset{i \in V}{\text{max}} \text{ } \omega(i, i+1)$$
$$\underset{i \ne j}{\text{min}} \text{ max} \text{ } \{\omega(i, j), \text{ }\omega(j, i)\}$$
...
---
# Flag Persistence (Unforced vs forced Lorenz)
...
![:img 30%, 5%, 35%](../figs/shared_features_lorenz_1.png)
--
![:img 30%, 35%, 35%](../figs/shared_features_lorenz_2.png)
--
![:img 30%, 65%, 35%](../figs/shared_features_lorenz_3.png)
...
---
# Future Directions (To do: turn this pictorial)

- Investigating effects of changing hyperparameters of the binning technique
- High-dimensional climate data
- Exploring recurrent, absorptive and transient classes using markov chains from the digraphs
